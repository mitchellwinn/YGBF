...
func minigame_countdown():
	...
	while countdown>0:
		...
		if is_equal_approx(countdown,5.0-random_floats[0]):
			spawn_key()
			print(str(countdown))
			continue
		if is_equal_approx(countdown,4.5-random_floats[1]):
			spawn_key()
			print(countdown)
			continue
		if is_equal_approx(countdown,4.0-random_floats[2]):
			spawn_key()
			print(countdown)
			continue
		if is_equal_approx(countdown,3.5-random_floats[3]):
			spawn_key()
			print(countdown)
			continue
		if is_equal_approx(countdown,3.0-random_floats[4]):
			spawn_key()
			print(countdown)
			continue

...
func ego_temp_armor() -> int:
	return 5*current_user.active_talent.main_attack_ego

func hp_temp_armor() -> int:
	return 5*current_user.active_talent.main_attack_hp
...
func use(user: Stats, target: Stats):
	...
	if BattleManager.rng.randi()%100<=accuracy:
		if BattleManager.rng.randi()%100<=crit_chance:
			...
			user.active_talent.main_attack_resource_limit -= user.active_talent.main_attack_resource_count
		...
		user.active_talent.main_attack_resource_limit -= 1
